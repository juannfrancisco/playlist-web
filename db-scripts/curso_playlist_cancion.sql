...
CREATE TABLE `playlist_cancion` (
  `idplaylist_cancion` int(11) NOT NULL AUTO_INCREMENT,
  `id_playlist` int(11) DEFAULT NULL,
  `id_cancion` int(11) DEFAULT NULL,
  PRIMARY KEY (`idplaylist_cancion`),
  KEY `fk_cancion_playlist_idx` (`id_cancion`),
  KEY `fk_playlist_id_idx` (`id_playlist`),
  CONSTRAINT `fk_cancion_playlist` FOREIGN KEY (`id_cancion`) REFERENCES `cancion` (`idcancion`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  CONSTRAINT `fk_playlist_id` FOREIGN KEY (`id_playlist`) REFERENCES `playlist` (`idplaylist`) ON DELETE NO ACTION ON UPDATE NO ACTION
